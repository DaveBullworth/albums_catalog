...
const Card = ({
  ...
  return (
    <div
      ...
      style={{ margin: isExpanded ? '0' : '0 0 10px' }}
    >
      <div
        className={`card ripple ${isExpanded ? 'expanded' : ''}`}
      ...
      >
        ...
        <div className="block" style={{ fontSize: '1.5rem' }}>
          <div className="icon1">
            {icon1 ? (
              <i className={`bi bi-heart-fill ${isColorDark ? 'text-white' : ''}`}></i>
            ) : (
              <i className={`bi-hand-thumbs-down-fill ${isColorDark ? 'text-white' : ''}`}></i>
            )}
          </div>
          <div className="icon2">
            {icon2 ? <i className={`bi bi-star-fill ${isColorDark ? 'text-white' : ''}`}></i> : ''}
          </div>
        </div>
        <div className="block">
          <button
            type="button"
            className={`${isColorDark ? 'btn btn-outline-light' : 'btn btn-outline-dark'}`}
          ...
          <i
            className={`bi bi-triangle-fill ${isExpanded ? 'rotate' : ''} ${isColorDark ? 'text-white' : ''}`}
          ></i>
        </div>
      </div>
    </div>
